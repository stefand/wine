...
void state_init(struct wined3d_state *state, const struct wined3d_gl_info *gl_info,
        const struct wined3d_d3d_info *d3d_info, DWORD flags)
{
    ...
    if (!(state->fb.render_targets = wined3d_calloc(gl_info->limits.buffers, sizeof(state->fb.render_targets))))
    {
        ERR("Out of memory\n");
        return;
    }
        ...
        struct wined3d_device *device, enum wined3d_stateblock_type type)
{
    ...
    state_init(&stateblock->state, gl_info, d3d_info, 0);
